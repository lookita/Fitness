...
export class PrismaService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor() {
    ...
    try {
      // Intentamos cargar el cliente real por si acaso
      const { PrismaClient } = require('@prisma/client');
      this.client = new PrismaClient();
    } catch (e) {
      console.warn('PrismaService: Usando modo emulación SQL por fallo de PrismaClient.');
      this.client = { $queryRaw: async (sql: string, ...args: any[]) => (await this.pool.query(sql, args)).rows };
    }

    // Retornamos un Proxy para emular los modelos si no existen
    return new Proxy(this, {
      get: (target, prop) => {
        if (prop in target) return (target as any)[prop];
        
        // Si el cliente real tiene el modelo, lo usamos (ej: client.usuarios)
        if (this.client && this.client[prop]) return this.client[prop];

        // Si no, devolvemos un MockModel que usa SQL directo
  ...
  private createMockModel(modelName: string) {
    return {
      ...
      findUnique: async (args: any) => {
        ...
        // Manejamos claves compuestas (@@id)
        const keys = Object.keys(where);
        let sql = `SELECT * FROM "${modelName}"`;
        const params: any[] = [];

        if (keys.length === 1 && typeof where[keys[0]] === 'object') {
          // Caso id_usuario_id_ejercicio: { id_usuario: 1, id_ejercicio: 2 }
          const compoundKeys = Object.keys(where[keys[0]]);
          sql += " WHERE " + compoundKeys.map((k, i) => `"${k}" = $${i + 1}`).join(" AND ");
      ...
      create: async (args: any) => {
        ...
        const sql = `INSERT INTO "${modelName}" (${keys.map(k => `"${k}"`).join(", ")}) VALUES (${keys.map((_, i) => `$${i + 1}`).join(", ")}) RETURNING *`;
      ...
      update: async (args: any) => {
        ...
        let sql = `UPDATE "${modelName}" SET ` + dataKeys.map((k, i) => `"${k}" = $${i + 1}`).join(", ");
        sql += " WHERE " + whereKeys.map((k, i) => `"${k}" = $${i + i + 1 + dataKeys.length}`).join(" AND ");
        const params = [...dataKeys.map(k => args.data[k]), ...whereKeys.map(k => args.where[k])];
        const res = await this.pool.query(sql, params);
        return res.rows[0];
      },
      upsert: async (args: any) => {
        // Simplificación: intentamos buscar, si existe update, si no create
        const existing = await this.createMockModel(modelName).findUnique({ where: args.where });
        if (existing) {
          return this.createMockModel(modelName).update({ where: args.where, data: args.update });
        } else {
          return this.createMockModel(modelName).create({ data: args.create });
        }
      },
      aggregate: async (args: any) => {
        // Solo implementamos _sum para ejercicios
        if (args._sum) {
           const field = Object.keys(args._sum)[0];
           const res = await this.pool.query(`SELECT SUM("${field}") as total FROM "${modelName}"`);
